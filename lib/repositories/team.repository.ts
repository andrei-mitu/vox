...
import type { Team, NewTeam, TeamMember, NewTeamMember } from '@/lib/db/schema';
...
export async function findTeamsByUserId(userId: string): Promise<Team[]> {
  return db()
    .select({ id: teams.id, name: teams.name, slug: teams.slug, logoUrl: teams.logoUrl, visibility: teams.visibility, createdAt: teams.createdAt, updatedAt: teams.updatedAt })
    .from(teams)
