...
function StatCard({ label, value, icon, href, accent }: StatCardProps) {
    return (
        ...
        >
            <div className={`flex items-center justify-center w-10 h-10 rounded-lg ${accent ? 'bg-accent-muted text-accent-primary' : 'bg-background-muted text-text-muted'}`}>
                {icon}
            </div>
            <div>
...
export default async function AdminDashboardPage() {
    ...
    return (
        <div className="p-8 max-w-5xl">
            ...
            {pending.length === 0 ? (
                <div className="flex items-center justify-center h-24 rounded-xl border border-border-default bg-background-secondary">
