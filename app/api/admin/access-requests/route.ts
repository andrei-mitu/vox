...
const VALID_STATUSES: AccessRequest['status'][] = ['pending', 'approved', 'rejected'];
...
export async function GET(request: Request): Promise<Response> {
    try {
        ...
        const rawStatus = (searchParams.get('status') ?? 'pending') as AccessRequest['status'];
        if (!VALID_STATUSES.includes(rawStatus)) {
            return ApiResponse.badRequest('Invalid status. Use: pending, approved, or rejected.');
        }
