...
export function CarrierDialog({
    ...
    const { form, set, submitting, isEditing, handleOpenChange, handleSubmit } =
        useDialogForm<CreateCarrierInput, CarrierDto>({
            entity: carrier,
            defaultForm,
            entityToForm: carrierToForm,
            createEndpoint: `/api/${ workspaceSlug }/carriers`,
            updateEndpoint: carrier ? `/api/${ workspaceSlug }/carriers/${ carrier.seqId }` : undefined,
            onSuccess,
            onOpenChange,
        });
